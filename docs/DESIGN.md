...
| 16 | TLS 指纹伪装（JA3/JA4） | §6.6 |
| 17 | 认证/连接 DoS 防护 | §9.2 |
| 18 | UDP 支持（SOCKS5 UDP ASSOCIATE） | §7.4 |
| 19 | 多跳中继（chain） | §7.1 |
| 20 | 弱网多协议/多伪装并发 | §6.7 |
| 21 | 多服务器冗余 + 故障转移 | §5.5 |
...
| `Connect {stream, target?, service?, via?, proto?, carrier?}` | 双向 | 申请建立数据流（`proto`：`tcp`/`udp`；`via`：单节点或链 `"a,b"`） |
...
| `Ack {stream, seq}` | 双向 | 滑动窗口流控回执（§7.5） |
...
### 5.4 加密复杂度取舍（避免过度加密特征）

外层由 nginx 提供 HTTPS/TLS 的真实机密性，因此**内部加密刻意从简**，避免「过度加密」导致流量特征更明显：

- 内部仅做一次 **AEAD 信封**（ChaCha20-Poly1305），用于完整性 + 认证 + 抗重放；不做多重加密、不做复杂密钥协商。
- **不引入 PFS / 会话密钥重协商**：会话密钥由 `secret` 经 HKDF 派生即可；`secret` 是受信部署内的预共享凭据，其泄露由密钥管理（运维侧）而非密码学复杂度兜底。
- **不引入端到端（B↔A）加密封层**：服务端作为可信枢纽可见明文（与 Xray 默认一致），再加一层只会抬高 CPU、增大包长、放大熵特征。
- **原则**：复杂度让给「伪装 / 随机化」而非「加密」——加密够用即可，重点是让流量看起来像普通 HTTPS 流量。

### 5.5 多服务器冗余与故障转移

- 客户端配置支持**多服务器列表**：`servers = ["https://a.example.com", "https://b.example.com"]`（替代单一 `server`）。
- **健康检查**：对每个服务器周期性探测（认证 / 心跳 / 延迟），维护健康度评分。
- **故障转移**：当前服务器认证失败 / 超时 / 断线时，按评分自动切换下一服务器，复用 §5.3 退避重连；切换对上层 SOCKS5 透明。
- **选择策略**：可配 `随机 / 优先级 / 延迟最低`。
...
### 6.6 TLS 指纹伪装（JA3 / JA4）

GFW 用 TLS 握手指纹（JA3/JA4）识别「工具型 TLS 流量」（附录 A.1）。客户端若用框架默认 TLS 指纹（reqwest/rustls 默认），与浏览器差异明显，易被识别：

- **客户端 TLS 指纹伪装**：客户端 TLS 栈采用 **uTLS 式指纹**，模拟主流浏览器（Chrome / Firefox / Safari / Edge）的 JA3/JA4，且每会话随机选其一。
- **服务端侧**：由 nginx 终结 TLS，服务端指纹天然是「正常 HTTPS 服务器」；wsnetd 不直接暴露 TLS。
- **回退**：指纹库随浏览器版本更新；无法精确伪装时回退到「贴近某主流浏览器」的近似指纹，优于框架默认指纹。

### 6.7 弱网策略：多协议 + 多伪装并发尝试

弱网（高丢包 / 高延迟 / 抖动）或某载体被针对性干扰时，单载体单点失败。策略是**同时多路并发 + 多形态混淆**：

- **多协议并发**：同一条控制消息（尤其建连握手 `Connect`/`Auth`）可**同时在多个载体**（POST / SSE / WS）发送副本；首个到达者生效，其余按 §4.2 的 counter 去重丢弃，保证语义「恰好一次」。弱网下「多条路一起试」显著提高成功率。
- **多伪装形态并发**：伪装流量同时混入**多种形态**——静态资源（html / js / css / 图片）、**动态 API**（`GET /api/*` 返回稳定 JSON）、表单 POST 等，避免单一形态的统计规律。
- **载体优先级降级**：弱网检测（丢包率 / RTT 阈值）触发后自动调整载体权重——WS 抖动严重时控制面更多走 POST 轮询；SSE 断开时降级为纯 POST 长轮询。
- **边界**：数据流仍固定走 WS 保序（§7.3）；多路径并发仅用于控制面与建连握手，数据流本身不跨载体复制。
- **动态 API 伪装**：内置 `/api/status`、`/api/user`、`/api/list` 等返回稳定 JSON 的端点，与 html/js/css 随机混搭。

### 6.8 其他混淆细节

- **包长伪装（padding，非加密）**：`Data` 帧随机填充至几档长度（如 256/512/1024），抹平包长统计特征；接收端按真实长度截取。仅做填充、不加加密层，符合「加密从简」（§5.4）。
- **时序抖动**：心跳与 POST 轮询间隔随机化，避免周期性时序特征。
- **空闲流量**：无业务时也维持低速率、多样化的伪装流量（§6.4），避免「有用户才来流量」的关联特征。
...
| 多跳中转（chain） | `host:port` | - | `"client-a,client-b"`（逗号分隔逐跳） |
...
- **多跳中继（chain）**：`via` 支持逗号分隔的链式列表（如 `"client-a,client-b"`），数据路径为 `client → server → client-a → server → client-b → 目标`。服务端逐跳编排：先让末端节点拨号目标，再逐层建立「经由下一跳」的中继，最后与请求方对接；每跳都是 §7.1 的 `(流, 流)` 中继映射，复用同一机制，跳数上限（如 4）防环路。
...
### 7.4 UDP 支持（SOCKS5 UDP ASSOCIATE）

> **SOCKS5 是否支持 UDP？支持**——RFC 1928 定义 `UDP ASSOCIATE`（0x03）命令。wsnet v1 通过「UDP-over-流」在 TCP 隧道上承载 UDP。

- **入站**：客户端 SOCKS5 处理 `UDP ASSOCIATE`，本机绑定 UDP 中继套接字，向 SOCKS5 客户端回送中继地址。
- **封装**：每个 UDP 数据报（含 SOCKS5 UDP 头 `RSV/FRAG/ATYP/ADDR/PORT`）作为一个 `Data` 帧发送；`Data` 帧天然按消息分界，故**一帧 = 一数据报**，无需额外长度前缀。
- **流类型**：`Connect` 增加 `proto` 字段（`"tcp"`（默认）/ `"udp"`）；`proto=udp` 的流在出口节点以 UDP 套接字收发，不做 TCP 半关闭（无连接语义，靠空闲超时回收）。
- **路由**：UDP 数据报按目的地址走同一路由器（§6），支持经节点 / 服务端出口。
- **局限**：UDP over TCP 存在丢包放大与乱序风险（可靠传输携带实时数据），对高实时 / 高丢包场景不理想；这是「全协议伪装 + 单一 TCP 隧道」的既定权衡，原生 UDP 隧道列为未来工作（§13）。

### 7.5 流量控制（滑动窗口）

v1 的「有界通道 + 溢出断开」会丢数据，不适用于要求可靠传输的流。改进为**滑动窗口背压**：

- 每条流维护发送窗口（如 64 帧）；对端通过 `Ack {stream, seq}` 控制消息回执，窗口滑动后才继续发送。
- 窗口耗尽即暂停该流发送（对上层 TCP 形成自然背压），而非丢帧。
- `Ack` 属控制消息，走控制载体（随机分发 §6）；数据仍固定走 WS 保序。
- 实现上可先以「有界通道 + 溢出断开」起步，滑动窗口作为增量升级。
...
| DoS / 资源耗尽 | 认证限速 + 连接上限 + 可选 PoW + 会话 TTL（§9.2） |
...
- **认证限速**：按来源 IP 对 `Auth` 做令牌桶限速；超限即静默丢弃或按 §9.1 伪装响应，不返回「限速」特征。
- **连接数上限**：单 IP 的 `/w`、`/e`、`/m` 并发连接数上限；超限拒绝（伪装成正常服务器行为）。
- **可选 PoW**：高防场景可开启轻量工作量证明（客户端在 `Auth` 附带 PoW 解），提高伪造握手成本。
- **校验成本控制**：HMAC/AEAD 均为对称原语、成本低；对非法请求**先做廉价检查**（格式 / 长度）再进入昂贵校验，避免 CPU 放大。
- **资源回收**：会话 TTL、空闲超时、流上限，防止僵尸连接占满内存。
...
node_id      = "client-a"
secret       = "change-me-a"
servers      = ["https://a.example.com", "https://b.example.com"]  # 多服务器冗余 + 故障转移（§5.5）
socks_listen = "127.0.0.1:1080"
encrypt_data = true
udp_enabled  = true                   # SOCKS5 UDP ASSOCIATE（§7.4）

[router]
final = "server"               # direct | server | <node-id> | "a,b"（多跳 chain）
...
- **UDP / 多跳 / 冗余**：
  - SOCKS5 `UDP ASSOCIATE` 数据报往返（含经节点出口）；
  - 多跳 chain（`via="a,b"`）数据往返；
  - 多服务器故障转移：主服务器宕机 → 自动切备用 → 会话级不中断；
  - 弱网并发：模拟丢包 / 延迟，验证多载体并发 + 去重语义正确。

## 13. 限制与未来工作

- **传输层约束**：v1 仅走 TCP + TLS（WS/SSE/POST），**不使用 QUIC/HTTP3**——规避当前最活跃的 QUIC-SNI 解密封锁面（见附录 A.3）；未来可评估 ECH 或 QUIC 载体。
- **原生 UDP 隧道**：v1 用「UDP-over-TCP」承载（§7.4）；原生 UDP / QUIC 数据报隧道列为未来工作。
- **数据面多载体随机化**：`data_mix` 开启后，每流可随机选「WS / POST 分块上传 + SSE 下载」，实现数据面的载体随机（需每流重排缓冲）。
- **端到端加密 / PFS**：**有意不做**（§5.4），避免过度加密抬高特征；若未来要求服务端不可见，再评估节点间协商。
- **节点级动态路由**：`PeerList` 已就绪，未来做基于负载 / 延迟的自动选路与故障转移（服务器级冗余已由 §5.5 覆盖）。
