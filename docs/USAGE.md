...
> 状态：本文是**接口契约**。`wsnet`/`wsnetd` 二进制、配置 schema、控制面与 Local Forward 均已实现并有测试；实现进度与已知缺口见 [README](../README.md)。
>
> 本文写于仓库还只有文档时，因此个别条目描述的是目标行为；凡是与 README 冲突的地方，以 README 列出的"已知缺口"为准。
